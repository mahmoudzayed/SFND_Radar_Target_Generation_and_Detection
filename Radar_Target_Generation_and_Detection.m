...
%the Bsweep of chirp for 1 m resolution
...
for i=1:length(t)         
    ...
    dt = t(i) - td(i);
    Rx(i) = cos(2*pi*(fc * dt + slope*(dt^2)/2));
...
for i = 1+Gr+Tr:Nr/2-(Gr+Tr)
   ...
   % CFAR
       lenR = Gr + Tr;
       lenD = Gd + Td;
%        disp([i,j,j-Gd,j+Gd, i-Gr, i+Gr]);
       Gwindow = sum(db2pow(RDM(i-Gr:i+Gr,j-Gd:j+Gd)),'all');
       Twindow = sum(db2pow(RDM(i-lenR:i+lenR,j-lenD:j+lenD)),'all');
     
       noise_threshold = pow2db((Twindow - Gwindow)/((2*lenR+1)*(2*lenD+1)- (2*Gr+1)*(2*Gd+1)));
       noise_threshold = noise_threshold + offset;
       % 6. Measuring the signal within the CUT
        CUT = RDM(i,j);

        % 8. Filter the signal above the threshold
        if noise_threshold < CUT
            CFAR(i,j) = 1.0;
        end
   end
end
